...
for (i in 1:2)
  lines(pred.temp, "slices", var = c(0, 32)[i], col = i + 2, lwd = 1.5)
...
basis.temp2 <- crossbasis(
  chicagoNMMAPS$temp,
  argvar = list(fun = "poly", degree = 6),
  arglag = list(knots = klag),
  lag = 30
)
...
basis.temp3 <- crossbasis(
  chicagoNMMAPS$temp,
  argvar = list(fun = "thr", thr.value = 25, side = "d"),
  arglag = list(knots = klag),
  lag = 30
)
...
legend(
  "top",
  c("natural spline", "polynomial", "double threshold"),
  col = 2:4,
  lty = c(1:2, 4),
  lwd = 1.5,
  inset = 0.1,
